...
NOTE: The Morse code table is preloaded for you as a dictionary, feel free to use it:

*/

function decodeMorse(code) {
  const words = code.replace('   ', ' _ ');
  let letters = String(words)
    .split(' ')
    .map(x => MORSE_CODE[x]);
  letters.forEach(function(e) {
    if (e === undefined) letters.splice(letters.indexOf(undefined), 1, ' ');
  });
  return letters
    .join('')
    .replace(/\s+/g, ' ')
